...
fn main() {
    ...
    let out_resize_natr_filename = r"barbara_256x256_resize_natr.y";
    let out_resize_neon_filename = r"barbara_256x256_resize_neon.y";
    let ref_filename = r"../out_c_natc_resize_barbara_256x256.y";
    ...
    let mut a_out_resize_y_natr_u8: Vec<u8> = vec![0; OUT_WID * OUT_HEI];
    ...
    loop {
        ...
        resize::run_resize_natr(&a_out_y_u8, &mut a_out_resize_y_natr_u8);
    ...
    let mut out_file = File::create(out_resize_natr_filename).expect("failed to open out resize file");
    
    out_file.write_all(&a_out_resize_y_natr_u8).expect("failed to write file");

    let mut a_out_resize_y_neon_u8: Vec<u8> = vec![0; OUT_WID * OUT_HEI];
    ...
    loop {
        ...
        resize::run_resize_neon(&a_out_y_u8, &mut a_out_resize_y_neon_u8);
    ...
    let mut out_file = File::create(out_resize_neon_filename).expect("failed to open out resize file");
    
    out_file.write_all(&a_out_resize_y_neon_u8).expect("failed to write file");

    for j in 0..OUT_HEI {
        for i in 0..OUT_WID {
            let natr_value = a_out_resize_y_natr_u8[i + (j * OUT_WID)];
            let neon_value = a_out_resize_y_neon_u8[i + (j * OUT_WID)];
